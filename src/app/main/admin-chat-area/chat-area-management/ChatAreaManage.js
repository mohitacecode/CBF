...
function ChatAreaManage(props) {
  ...
  const [roomId, setRoomId] = useState(contact.rooom_id);
    ...
    if (!contactSidebarOpen) {
      ...
      if (contact.bot_is_active) {
        BotDetailService.getSessionVariables({ pathParam: { room_id: contact.room_id } })
          ...
          .catch(err => {
            console.log(err);
            dispatch(showMessage({ message: "Failed to Get Variables", variant: "error" }));
          });
      } else {
          ...
          <Hidden mdUp>
            ...
            >
              <AppBar position="static">
                ...
                >
                  <Tab label="Normal" />
                  <Tab label="Agent Takeovers" />
                </Tabs>
              </AppBar>
              <TabPanel value={activeTab} index={0} style={{ "overflow-y": "scroll" }}>
                <ChatsSidebar
              ...
              <TabPanel value={activeTab} index={1} style={{ "overflow-y": "scroll" }}>
                <ChatsSidebar
                  takeover
                  selectedIndex={selectedIndex}
                  setSelectedIndex={setSelectedIndex}
...
export default withReducer("chat", reducer)(ChatAreaManage);
