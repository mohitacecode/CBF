...
function ContactSidebar({ close, review, variables, loading }) {
          ...
          <li className={classes.listSection}>
            <ul className={classes.ul}>
              {review && review.length > 0 ? (
                <React.Fragment>
                  <ListSubheader>Chat Reviews</ListSubheader>
                  {Object.entries(review).map((rev, index) => (
                    <ListItem key={"rev" + index}>
                      <ListItemText
                        primary={
                          <Typography variant="subtitle2" component="h6">
                            {rev[1].is_admin
                              ? "Admin: " + rev[1].operator_email
                              : "Operator: " + rev[1].operator_email}
                            <Typography variant="caption" component="h6">
                              {"Action: " + rev[1].close_chat_action}
                            </Typography>
                            <Typography variant="caption" component="h6">
                              {"Note: " + rev[1].review_note}
                            </Typography>
                          </Typography>
                        }
                      />
            ...
            <ul className={classes.ul}>
              {variables && _.size(variables) === 0 && review && review.length === 0 ? (
